...
meta = pd.read_csv('/vol/bitbucket/apg416/cv-corpus-5.1-2020-06-22/en/validated.tsv', sep='\t')
meta = meta[((meta['gender'] == 'male') | (meta['gender'] == 'female')) & (meta['segment']!= 'Singleword Benchmark')]
meta = meta.drop_duplicates(subset=['client_id'])
metaf, metam = meta[meta['gender']=='female'], meta[meta['gender']=='male'].sample(2807)
...
for idx, fileName in enumerate(meta.path):
    ...
    np.save(os.path.join(targetDir_emb, fileName[:-4]),
